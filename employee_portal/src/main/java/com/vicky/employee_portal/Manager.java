...
public class Manager {
	...
	void statusChangeRequests() {
		...
		String date = sc.next();
		Query query1 = manager.createQuery("from Employee_Leave where Employee_ID =:id and Leave_Date=:date")
				.setParameter("id", id).setParameter("date", date);
		List<Employee_Leave> statusid = query1.getResultList();
		int primary = statusid.get(0).S_No;
		try {
			Employee_Leave leave = manager.find(Employee_Leave.class, primary);
